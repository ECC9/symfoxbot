exports.run = (message, bot) => {
  if (!message.mentions.users.first()) return message.channel.send("**Mention a user or multiple users to kick.**")
    ...
    setTimeout(function() {
        ...
        message.channel.send(text.join(", ")+" **has been kicked from the server.**", {split:true});
    }, 1000);
}
